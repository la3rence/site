...
export const getMdContent = (id, mdDirectory) => {
  // default using the `./posts` for markdown directory
  if (!mdDirectory) {
    mdDirectory = path.join(process.cwd(), "posts");
  }
  const mdPostNames = fs.readdirSync(mdDirectory);
  const mdPostsData = mdPostNames
    ...
    .map(async mdPostName => {
      ...
      const htmlResult = await remark()
        ...
        .use(
          rehypeWrap,
          // wrap these DOMs for -margin-x style makes them extend to the sides
          ["img", "pre", "table", "video"].map(dom => {
            return {
              selector: dom,
              wrapper: "div.-mx-6",
            };
...
export const getAllPostData = async () => {
  ...
  const mdPostsData = getMdPostsData(mdDirectory);
  const jsDirectory = path.join(process.cwd(), "pages/blog");
  const jsPostsData = await getJsPostsData(jsDirectory);
