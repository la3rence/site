...
function search(keyword, allPosts, contextLength) {
  keyword = keyword.replace(/[/\-\\^$*+?.()|[\]{}]/g, "\\$&"); // escape
  const results = {};
  for (const post of allPosts) {
    let match;
    let lastIndex = -1;
    const context = post.title + post.description + post.content;
  ...
  const finalResults = [];
  const highlightRegExp = new RegExp(keyword, "gi");
  const highlightDom = "<mark>" + keyword + "</mark>";
  for (const post of allPosts) {
    if (results[post.id]) {
      ...
      finalResults.push({
        id: post.id,
        title: post.title.replace(highlightRegExp, highlightDom),
        contexts: `...${combined}...`.replace(highlightRegExp, highlightDom),
      });
    }
  }
  return finalResults;
