...
axios.defaults.timeout = 5000; // timeout for rehype gist
...
export const getMdContentById = async (id, mdDirectory, withHTMLString = true) => {
  ...
  try {
    htmlResult = await renderHTMLfromMarkdownString(matterResult.content, isBlog, fileInfo.locale, {
      hasGist,
    });
  } catch (error) {
    if (hasGist) {
      console.warn("Gist fetch failed for %s, retrying without gist: %s", id, error.message);
      gistFailed = true;
      htmlResult = await renderHTMLfromMarkdownString(
  ...
  const postData = {
    ...
    hasGist: gistFailed ? false : hasGist,
    hasAlert,
...
const renderHTMLfromMarkdownString = async (
  ...
  return processor
    .use(rehypeShiki, {
      ...
      langs: ["javascript", "jsx", "bash", "c", "go", "java", "json", "xml", "yaml", "text"],
