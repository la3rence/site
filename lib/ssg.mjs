...
export const getMdPostsIndexData = mdDirectory => {
  ...
  if (cache.has("ssg:mdpostindex:" + mdDirectory) && IS_PROD) {
    console.log("cache hit: markdown post index", mdDirectory);
    return cache.get("ssg:mdpostindex:" + mdDirectory);
  }
  ...
  const filteredData = mdPostsData.sort(sortByDate).filter(filterVisibility);
  cache.set("ssg:mdpostindex:" + mdDirectory, filteredData);
  return filteredData;
};
...
export const getMdContentById = (id, mdDirectory, withHTMLString = true) => {
  ...
  const mdPostsData = mdPostNames
    ...
    .map(async mdPostName => {
      ...
      const alertPatterns = ["[!NOTE]", "[!TIP]", "[!IMPORTANT]", "[!WARNING]", "[!CAUTION]"];
      const hasAlert = alertPatterns.some(pattern => mdContent.includes(pattern));
      return {
        id: idWithoutLocale,
...
export const getAllPostIndexData = async () => {
  const mdPostsData = getMdPostsIndexData();
  const jsPostsData = await getJsPostsData(defaultJSDirectory);
  const all = [...mdPostsData, ...jsPostsData];
  return all.sort(sortByDate).filter(filterVisibility);
};
