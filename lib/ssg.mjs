...
export const getMdContentById = (id, mdDirectory, withHTMLString = true) => {
  ...
  const mdPostsData = mdPostNames
    ...
    .map(async mdPostName => {
      ...
      const htmlResult = await renderHTMLfromMarkdownString(matterResult.content, isBlog);
      return {
        id: idWithoutLocale,
...
const blogTOC = (toc, isBlog) => {
  toc.properties = { id: ["toc"], class: "toc" };
...
const renderHTMLfromMarkdownString = async (markdownString, isBlog) => {
  return remark()
    ...
    .use(rehypeSlug)
    .use(rehypeToc, { nav: true, customizeTOC: toc => blogTOC(toc, isBlog) })
