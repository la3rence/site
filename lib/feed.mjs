import { getMdContentById, getMdPostsData, defaultMarkdownDirectory } from "./ssg.mjs";
...
const buildFeed = async () => {
  ...
  locales?.forEach(locale => {
    ...
    const minifiedFeedString = minifyXML(feed);
    const fileName = `./public/atom.${locale}.xml`;
    fs.openSync(fileName, "w");
    fs.writeFileSync(fileName, minifiedFeedString);
    if (locale === defaultLocale) {
      // just an alternate for default locale feed (to keep old link working)
      fs.writeFileSync(`./public/${feedFile}`, minifiedFeedString);
    }
  });
};
...
function mapToAtomEntry(post) {
  ...
  const postContent = `${post.htmlStringContent}${createFeedActionLinks(post.locale)}`;
  return `<entry>
      <title>${decode(post.title)}</title>
      <id>${postLink}</id>
      <link href="${postLink}"/>
      <published>${post.date}T00:00:00.000Z</published>
      <updated>${post.modified ? post.modified : post.date}T00:00:00.000Z</updated>
      ${categories
        .map(tag => {
        ...
        .join("")}
      <summary type="html">${decode(postContent)}</summary>
      <author><name>${post.author}</name></author>
    </entry>`;
}

function createFeedActionLinks(locale) {
  const translations = getTranslations(locale);
  return `<p><a href="mailto:${authorEmail}">${translations["Write Me"]}</a> <a href="${rewardURL}">${translations["Tip Me"]}</a></p>`;
}
...
function createRSS(blogPosts, locale) {
  ...
  <feed xmlns="http://www.w3.org/2005/Atom">
    ...
    <follow_challenge>
        <feedId>55815884011044881</feedId>
        <userId>189366433304344576</userId>
    </follow_challenge>
    <author>
